...
int main(void)
{
    ...
    while (TRUE) {
        if (AdcResult.event == TRUE) {
            // UPDATE OLED:
            // Percent code:
            float percentage = (AdcResult.value / 1023.0)*100.0;

            //OledDrawString("Potentiometer Value: \n");
            sprintf(pt, "Potentiometer Value: \n%4d \n %3.0f %%", AdcResult.value, (double) percentage);
            OledDrawString(pt);
            OledUpdate();
...
void __ISR(_ADC_VECTOR, IPL2AUTO) AdcHandler(void)
{
    ...
    int previousVal = AdcResult.value;
    AdcResult.value = (ADC1BUF0 + ADC1BUF1 + ADC1BUF2 + ADC1BUF3 + ADC1BUF4 + ADC1BUF5 + ADC1BUF6 + ADC1BUF7) / 8;

    if (previousVal != AdcResult.value) {
